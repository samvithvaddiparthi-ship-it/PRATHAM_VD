...
def correct(raw: str, lang: str, patient_name: str = "") -> dict:
    ...
    used_llm = False
    llm_ok = False   # the medical-correction LLM was reachable AND responded
    try:
        llm_out = llm_validate(text, lang, lex_changes, protected)
        llm_ok = llm_out is not None
...
def correct_light(raw: str, lang: str, patient_name: str = "") -> dict:
    """Deterministic-only correction — NO LLM, NO de-stutter. Applies just the two
    strategies that actually help English: the drug/lab-name lexicon (fuzzy) and
    the patient-name credential match. Used for English answers, where the Hindi/
    Telugu symptom lexicon and the LLM validation add nothing."""
    text, lex_changes = apply_lexicon(raw, lang)            # drugs/labs/EN-term casing
    text, name_changes = apply_name(text, patient_name, lang)  # snap to credential spelling
    changes = lex_changes + name_changes
    return {
        "raw": raw,
        "corrected": text,
        "confidence": None,
