...
4. Questionnaire (DAG)
Question content: Should the DAG questions be generic placeholders for testing now, or real clinical questions already — given we don't yet know the exact departments? (template already provided)
...
Residency: On-shore vs cloud for patient PHI(Protected Health Information) , and hosting model (on-prem at hospital vs Indian-region cloud)?
Auth model: Per-user accounts + RBAC(Role-Based Access Control) ? SSO(Single Sign-On) ? (we've added shared-passcode admin login + JWT roles as an interim step)
Patient identity: Do we match an existing hospital ID (UHID(Unique Health ID) /MRN(Medical Record Number )) or mint our own?
Compliance: Required consent, data retention, deletion, and audit-trail policies?

8. Interoperability & regulation
HIS(Unique Health ID) / HMIS integration: Which existing system must we integrate with? Is ABDM/ABHA (national health ID) in scope?
Regulatory: Is ethics-committee / IRB approval needed for the pilot? Any CDSCO / medical-device (SaMD) Software as a Medical Device   considerations, given triage influences clinical decisions?
...
Takeaways from 26-6-2026 Meeting:
