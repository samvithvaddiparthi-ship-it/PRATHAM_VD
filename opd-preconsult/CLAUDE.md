...
- **Data privacy (India DPDP Act 2023):** patient data is sensitive PHI. Consent before collection (already in flow); minimize what's stored; **de-identify any data used for testing/benchmarking**; secure storage, access control, audit trail, and deletion support.
...
- **Auth / access control:** node-backend JWT is hardened — `dev_secret` removed (fails closed in production without a strong `JWT_SECRET`; uses a random ephemeral key in dev). Tokens carry a `role` (`patient`/`doctor`/`admin`) enforced by `requireRole` (`middleware/auth.js`). Mutating admin/doctor-management/analytics/protocol/prescription endpoints are now role-gated, and the HIS dashboard sits behind an admin passcode login (`POST /api/admin/login`, env `ADMIN_PASSCODE`). **Required env:** `JWT_SECRET` (strong) and `ADMIN_PASSCODE` (≥6 chars) — both in `.env` (gitignored), placeholders in `.env.example`. **Still open (release blockers):** python-backend endpoints (`/api/ocr|report|triage|scribe`) are unauthenticated (cross-service auth = phase 2); `DEMO_QR_SECRET` still has a weak default; admin is a single shared passcode (no per-user RBAC/SSO); no per-hospital tenancy.
